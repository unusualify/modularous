...
return new class extends Migration
{
    ...
    public function up()
    {
        $filesTable = modularousConfig('tables.files', 'modularous_files');
        $fileablesTable = modularousConfig('tables.fileables', 'modularous_fileables');
        ...
        if (! Schema::hasTable($fileablesTable)) {
            Schema::create($fileablesTable, function (Blueprint $table) use ($filesTable) {
                ...
                $table->timestamps();
                $table->softDeletes();
                $table->{modularousIntegerMethod()}('file_id')->unsigned();
                $table->foreign('file_id', 'fk_files_file_id')->references('id')->on($filesTable)->onDelete('cascade')->onUpdate('cascade');
                // $table->{modularousIntegerMethod()}('fileable_id')->nullable()->unsigned();
