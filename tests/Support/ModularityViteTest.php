...
class ModularityViteTest extends TestCase
{
    protected ModularityVite $vite;
    ...
    protected function setUp(): void
    {
        ...
        $this->vite = new ModularityVite;
        $this->buildPath = public_path('vendor/modularity');

        $this->ensureModularityBuildExists();
    }

    protected function ensureModularityBuildExists(): void
    {
        $distPath = realpath(__DIR__ . '/../../vue/dist/modularity');
    ...
    protected function createFixtureBuild(): void
    {
        ...
        $manifest = json_decode(
            File::get(__DIR__ . '/../fixtures/modularity-manifest.json'),
            true
        );
        File::put(
            $this->buildPath . '/modularity-manifest.json',
    ...
    public function test_uses_modularity_manifest_filename(): void
    {
        $reflection = new \ReflectionClass($this->vite);
        $prop = $reflection->getProperty('manifestFilename');
        $prop->setAccessible(true);

        $this->assertEquals('modularity-manifest.json', $prop->getValue($this->vite));
    }

    public function test_uses_modularity_build_directory(): void
    {
        $reflection = new \ReflectionClass($this->vite);
        $prop = $reflection->getProperty('buildDirectory');
        $prop->setAccessible(true);

        $this->assertEquals('vendor/modularity', $prop->getValue($this->vite));
    }
    ...
    public function test_asset_uses_custom_build_directory(): void
    {
        ...
        $url = $this->vite->asset($entrypoint, 'vendor/modularity');

        $this->assertIsString($url);
        $this->assertStringContainsString('vendor/modularity', $url);
    }
    ...
    public function test_invoke_in_hot_mode_prepends_svg_spritemap_client(): void
    {
        ...
        // ModularityVite prepends @vite-plugin-svg-spritemap/client and @vite/client (Laravel only prepends @vite/client)
    ...
    {
        $manifestPath = $this->buildPath . '/modularity-manifest.json';
        if (! is_file($manifestPath)) {
            $this->markTestSkipped('Modularity manifest not found');
        }
