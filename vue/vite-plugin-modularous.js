...
function resolvePluginConfig (config) {
  ...
  return {
    ...
    hotFile: config.hotFile ?? path.join(config.publicDirectory ?? 'public', 'modularous.hot'),
...
export default function modularous (config) {
  ...
  return {
    ...
    configureServer (server) {
      server.httpServer?.once('listening', () => {
        ...
        if (isAddressInfo(address)) {
          ...
          const viteDevServerFullUrl = (viteDevServerUrl + resolvedConfig.base).replace(/^\/|\/$/g, '') // 'http://jakomeet.test:8080/vendor/modularous'
          fs.writeFileSync(pluginConfig.hotFile, viteDevServerFullUrl)
          // fs.writeFileSync(pluginConfig.hotFile, 'http://jakomeet.test:8080/vendor/modularous')
    ...
    transform: (code, id) => {
      ...
      if (resolvedConfig.command === 'serve') { // means development
        ...
        transformedCode = transformedCode.replace(/__modularous_vite_placeholder__/g, viteDevServerUrl)
