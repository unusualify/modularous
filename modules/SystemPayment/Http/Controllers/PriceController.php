...
class PriceController extends Controller
{
    ...
    public function checkout(Request $request)
    {
        ...
        if (! isset($params['payment_service'])) {
            if ($request->ajax()) {
                return response()->json([
                    'status' => MessageStage::ERROR,
                    'message' => 'Not compatible payment service',
        ...
        if (! $paymentService->serviceClass::hasBuiltInForm()) {
            if ($request->ajax()) {
                return response()->json([
                    'status' => MessageStage::ERROR,
                    'message' => 'Payment service does not support built-in form',
                ], 403);
            }

            return redirect()->back()->with('error', 'Payment service does not support built-in form');
        }
