...
This command has `$hidden = true` and does not appear in `php artisan list`. It is run automatically during `modularity:install` and when upgrading between major versions.
:::

## Command Information

- **Signature:** `modularity:update:laravel:configs`
- **Category:** Update
...
1. **`config/auth.php`** — adds the Modularous guard and user provider definitions so that `Auth::guard('modularity')` resolves correctly.
