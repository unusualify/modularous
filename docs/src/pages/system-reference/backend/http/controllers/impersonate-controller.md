...
2. Stores the original admin's ID in `session('modularous.impersonate.original_id')`.
3. Logs in as the target user via the modularous guard.
...
2. Restores the admin user via the modularous guard.
