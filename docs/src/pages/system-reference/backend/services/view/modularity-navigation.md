---
sidebarPos: 4
sidebarTitle: ModularityNavigation
---

# ModularityNavigation

**File**: `src/Services/View/ModularityNavigation.php`

`ModularityNavigation` builds the sidebar navigation array that is passed to the frontend. It resolves route names, applies permission checks, resolves badge values, and marks the active item based on the current request URL.
...
| Method | Description |
|--------|-------------|
| `systemMenu()` | Returns the navigation array built from Modularity's system modules |
...
When a nav item is both active and has a numeric badge, `ModularityNavigation` applies contrasting badge styling:
