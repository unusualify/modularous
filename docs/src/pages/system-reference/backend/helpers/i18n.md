...
```php
modularityTrans(string $key, array $replace = [], string $locale = null): string
```

Looks up a translation key in the `modularity::` namespace. Returns the raw key if no translation is found, making missing keys visible rather than silently empty.
